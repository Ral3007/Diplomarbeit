...
\subsection{Use Case}
Der Modulbetreiber bekommt eine Anfrage vom Kunden mit dem eine just-in-Time Vereinbarung getroffen wurde. Die Produktion von xxx muss am nächsten Tag starten. Zurzeit befindet sich das benötigte Dosiermodul jedoch in der Anlage yy. Um etwas Vorlauf zu gewinnen wird in Absprache mit dem Assistenzsystem die aktuelle Produktion in Anlage yy auf 110\% gefahren. Bis zum Anlauf der Produktion am nächsten Tag muss jedoch eine Lösung gefunden werden. Die Optionen sind vielfältig.
\begin{itemize}
\item Die Produktion von xxx hat Vorang und die Anlage yy wird solange stillgelegt. Dies führt zu massiven Kosten, da nicht weiter produziert werden kann.
\item Es existiert noch ein weiteres Modul aa, dass eventuell verwendet werden kann. Das Dosiermodul aa hat einen geringeren Durchfluss und stellt andere Services zur Verfügung.
\item Es steht noch ein Modul bb zur Verfügung, das jedoch ausgetauscht wurde, da es zu viel Energie verbraucht. Hier ist auf die Auslastung des Energienetzes Rücksicht zu nehmen.
\item Auch die Möglichkeit ein entsprechendes Modul zu leihen besteht. 
\end{itemize}
...
Der sicherlich wichtigste Punkt ist die Kompatibilität. Passen die Anschlüsse zu den anderen Modulen meiner Anlage? Kann das Rezept weiterhin, wie konfiguriert gefahren werden oder sind Anpassungen notwendig?

Was kostet der Austausch? 

Wie viel Energie verbraucht mein Modul?
