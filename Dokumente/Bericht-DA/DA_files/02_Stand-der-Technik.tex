...
Die Prozessautomatisierung umfasst die Automatisierung technischer Prozesse und kann sich aus folgenden Bestandteilen zusammen setzen \cite{UrbasP2012} \todo{Quelle?}:
\begin{itemize}
...
Es gibt viele Faktoren, die den Prozess des Problemlösens beeinflussen. Es kann \todo{wird?} zwischen äußeren Faktoren, wie die Ausgangssituation und die verfügbaren Operatoren, und den inneren Faktoren, wie Motivation und Emotionen, unterschieden werden.
...
\item \textbf{Diagnose:} Das Assistenzsystem muss wissen, welche Effekte auftreten können, wenn der Nutzer \todo{Mensch?} fehlerhafte Eingaben tätigt.
\item \textbf{Korrektur:} Wenn die Handlung des Nutzers \todo{Menschen?} von den Anweisungen abweicht, so muss das Assistenzsysteme diese Handlung trotzdem unterstützen können.
\end{itemize}
...
\begin{tabular}{p{3cm}|p{3cm}|p{3,5cm}|p{4cm}|p{3,5cm}}
